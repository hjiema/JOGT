...
//					jobFGTList[gpuId] = (sumFGTOnGPU(root) - fgtOnGPU + 1000*math.Abs(float64(current.Runtime)-float64(jobRuntime))) * (float64(current.StartTime) + float64(jobRuntime))
//					root.Runtime = types.Time(jobRuntime)
//				} else {
//					jobFGTList[gpuId] = (sumFGTOnGPU(root) - fgtOnGPU) * (float64(current.StartTime) + float64(jobRuntime))
//				}
//			} else {
//				current := root.Left
...
//							jobFGTList[gpuId] = ((sumFGTOnGPU(root) + fgtOnGPU) * (float64(current.EndTime) + float64(jobRuntime)))
//							break
//						} else {
...
//						fgtOnGPU := fragOnGPU * (jobRuntime + float64(target.EndTime))
//						current.FGT = current.Frag * math.Abs(float64(current.Runtime)-float64(jobRuntime))
//						if target.Runtime < types.Time(jobRuntime) {
//							jobFGTList[gpuId] = sumFGTOnGPU(root) - fgtOnGPU + 1000*math.Abs(float64(current.Runtime)-float64(jobRuntime))
...
//					fragOnGPU := calculateFragRuntime(remainMilliOnGPU)
//					fgtOnGPU := fragOnGPU * (jobRuntime + float64(root.EndTime))
//					jobFGTList[gpuId] = (sumFGTOnGPU(root) + fgtOnGPU) * (float64(root.EndTime) + float64(jobRuntime))
//				}
//			}
//		}
//	}
...
//					}
//					target.FGT = target.FGT - fgtOnGPU
//					if target.Runtime < types.Time(jobRuntime) {
//						target.Runtime = types.Time(jobRuntime)
//					}
//					break
