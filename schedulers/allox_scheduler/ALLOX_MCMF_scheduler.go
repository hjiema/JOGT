...
func (a *AlloxScheduler) DoOneShotSchedule() {
	// hasDoneScheduledOnce默认为false，判断若该值为真，则直接返回不执行后续代码。
...
// 先在图中进行表示，接下来和框架进行整合
// 从MCMFSolver中提取调度结果，生成一个作业到GPU的映射。通过遍历路径和边，确定每个作业分配到哪个GPU。
func (solver *MCMFSolver) GetSchedulingResult() map[string]string {
	...
	for _, path := range solver.paths {
		for _, edge := range path {
			...
			// 边不反向边，则是作业 -> GPU. 将作业名映射from.name到GPU名to.name。
			if !edge.reversed {
				// Job -> GPU
