...
func initDataSource(csvFilePath string, dataSourceRange []int) {
	...
	for idx, header := range csvHeaders {
		if len(header) > 0 && util.StringSliceIndexOf([]string{colJobName, colSubmitTime, colGpuMilli, colDDL}, header) == -1 {
	...
	for _, record := range csvDataRecords {
		...
		gpuMilli, err := strconv.Atoi(record[colGpuMilliIdx])
		if err != nil {
			// 处理转换错误
			panic(err)
		}
		...
		jobMetas[jobName] = NewJobMeta(jobName, types.Time(submitTime), gpuMilli, types.Time(ddl), durations)

		// jobMetasWithMilli[jobName] = NewJobMetaWithMilli(jobName, types.Time(submitTime),, types.Time(ddl), durations)
...
func (ds *DataSource) IterBySubmitTime(iterFunc func(indices []int, meta []types.JobMeta)) {
	...
	for i := 0; i < len(iterTarget); i++ {
		// metas初始长度为0,初始容量为1. metas和indices切片,用于存储当前提交时间相同的任务元数据和索引.
