...
//		for i := 0; i < count; i++ {
//			l = append(l, NewGPU(gpuID, gpuType, gpuSourMIlli))
//			gpuID++
//		}
//		gpus[gpuType] = l
//	}
...
type Cluster struct {
	...
	timer               types.Time
	gpuJobQueues        map[types.GPUID]*GPUJobQueue
	finishJobs          map[types.GPUID][]types.Job
}
...
func NewCluster(gpuType2CountConfig map[types.GPUType]int) *Cluster {
	...
	for _, gpuTypeStr := range gpuTypeStrings {
		...
		l := make([]types.GPU, 0, count)
		for i := 0; i < count; i++ {
			// 创建 GPU，并将其 ID 添加到 gpuIDs 列表中
			newGPU := NewGPU(gpuID, gpuType)
			l = append(l, newGPU)
			gpuIDs = append(gpuIDs, gpuID) // 将 GPU ID 加入列表
			gpuID++
		}
		gpus[gpuType] = l
	}
...
func (c *Cluster) GPUIDs() []types.GPUID {
	return c.gpuIDs
}
...
func (c *Cluster) passDuration(duration types.Duration) []*Job {
	...
	// 当前时间点,即推进之前的时间点
	fromTime := c.timer
	// 更新当前时间点,将当前时间点推荐duration指定的时间长度
	c.timer += types.Time(duration)
	newlyFinishedJobs := make([]*Job, 0)
	wg := &sync.WaitGroup{}
	finishedJobsSlice := make([][]*Job, len(c.gpuJobQueues))
